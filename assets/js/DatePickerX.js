  ...
  function DPX(input) {
    ...
    function getMinDate() {
      ...
      if (value instanceof HTMLInputElement) {
        ...
        value =
          value === null
            ? options.minDate.DatePickerX.getMinDate()
            : new Date(value);
    ...
    function getMaxDate() {
      ...
      if (value instanceof HTMLInputElement) {
        ...
        value =
          value === null
            ? options.maxDate.DatePickerX.getMaxDate()
            : new Date(value);
        value.setDate(value.getDate() - 1);
      } else if (typeof value === "function") {
        value = new Date(value());
    ...
    function createElements() {
      ...
      elements.prevTitle = createElement(
        "button",
        "dpx-prev",
        titleBox,
        "&#x276e;"
      ...
      elements.nextTitle = createElement(
        "button",
        "dpx-next",
        titleBox,
        "&#x276f;"
      ...
      elements.content = createElement(
        "div",
        "dpx-content-box",
        elements.container
      );
      ...
      if (options.todayButton || options.clearButton) {
        ...
        options.todayButton &&
          (elements.today = createElement(
            "button",
            ["dpx-item", "dpx-today"],
            btns,
            options.todayButtonLabel,
            options.todayButtonLabel
          ));
        options.clearButton &&
          (elements.clear = createElement(
            "button",
            ["dpx-item", "dpx-clear"],
            btns,
            options.clearButtonLabel,
            options.clearButtonLabel
          ));
      }
    }
    ...
    function openDatePicker(e) {
      if (!isActive()) {
        ...
        if (
          bcr.bottom > window.innerHeight &&
          bcr.top + input.offsetHeight > elements.container.offsetHeight
        ) {
          elements.container.classList.add("to-top");
          elements.container.getBoundingClientRect().top < 0 &&
            elements.container.classList.remove("to-top");
        }
        ...
        Array.prototype.slice
          .call(
            document.querySelectorAll(
              "[href], input, select, textarea, button, iframe, object, embed, [tabindex], [contenteditable]"
            )
          )
    ...
    function closeDatePicker() {
      if (openedDPX) {
        ...
        Array.prototype.slice
          .call(
            document.querySelectorAll("[data-datepickerx-disabled-tab-index]")
          )
          .forEach(function (item) {
        ...
        Array.prototype.slice
          .call(openedDPX.querySelectorAll("button"))
          .forEach(function (item) {
            item.tabIndex = -1;
          });
    ...
    function addEvents(dpx) {
      ...
      !globalEventsInitiated &&
        window.addEventListener("keydown", function (event) {
          if (event.keyCode === 13) {
            ...
            var focusableElements = Array.prototype.slice.call(
              elements.container.querySelectorAll("button:enabled")
            );
            if (
              document.activeElement &&
              focusableElements.indexOf(document.activeElement) >= 0
            ) {
              document.activeElement.neighbors &&
                document.activeElement.neighbors[
                  arrowsKeyMap[event.keyCode]
                ].focus();
            } else {
      ...
      elements.content.addEventListener("click", function (e) {
        if (mode === 2) {
          ...
          e.target.dpxValue >= min.getTime() &&
            e.target.dpxValue <= max.getTime() &&
            ++mode &&
            draw(e.target.dpxValue);
        }
      });
    ...
    function draw(dt) {
      ...
      // today button
      options.todayButton &&
        (elements.today.disabled = current < dtMin || current > dtMax);
      ...
      elements.title.innerHTML = mode
        ? getFormattedDate(
            dt,
            mode === 2 ? options.titleFormatMonth : options.titleFormatYear
          )
        : zeroYear + " - " + (zeroYear + 9);
      elements.title.dpxValue = dt.getTime();
      elements.title.title =
        mode === 2
          ? getFormattedDate(dt, options.titleFormatYear)
          : zeroYear + " - " + (zeroYear + 9);
      ...
      mode === 2
        ? dt.setMonth(setMonth - 1)
        : dt.setFullYear(mode ? setYear - 1 : zeroYear - 10);
      ...
      mode === 2
        ...
        : dt.setFullYear(mode ? setYear + 1 : zeroYear + 20);
      ...
      mode === 2
        ...
        : dt.setFullYear(mode ? setYear : zeroYear);
      ...
      if (mode === 2) {
        ...
        !options.mondayFirst &&
          ...
          createElement(
            "span",
            ["dpx-item", "dpx-weekday", "dpx-weekend"],
            elements.content,
            options.weekDayLabels[6]
          );
      ...
      if (mode === 2) {
        ...
        dt.setDate(
          options.mondayFirst ? -(dayWeek ? dayWeek - 2 : 5) : -dayWeek + 1
        );
      } else {
      ...
      for (var i = rows * cols; i--; dt[setter](dt[getter]() + 1)) {
        var classes = ["dpx-item"],
          title = getFormattedDate(
            dt,
            [
              options.titleFormatYear,
              options.titleFormatMonth,
              options.titleFormatDay,
            ][mode]
          );

        (mode
          ? mode === 2
            ? dt.getMonth() !== setMonth
            : dt.getFullYear() !== setYear
          : dt.getFullYear() < zeroYear || dt.getFullYear() > zeroYear + 9) &&
          classes.push("dpx-out");
        mode === 2 &&
          (dt.getDay() === 6 || dt.getDay() === 0) &&
          classes.push("dpx-weekend");
        ...
        var content = mode
            ? mode === 2
              ? dt.getDate()
              : options.shortMonthLabels[dt.getMonth()]
            : dt.getFullYear(),
          el = createElement(
            "button",
            classes,
    ...
    function setNeighborElements(items, rows, cols) {
      ...
      var bottomElements = [];
      elements.today &&
        !elements.today.disabled &&
        bottomElements.push(elements.today);
      ...
      topElements.forEach(function (item, index, array) {
        ...
        item.neighbors.top = bottomElements.length
          ? bottomElements[0]
      ...
      bottomElements.forEach(function (item, index, array) {
        ...
        item.neighbors.bottom = topElements.length
          ? topElements[0]
          : focusableElements[0];
      });
      ...
      function getTopLineElement(index) {
        ...
        return [
          ...
          elements.prevTitle.disabled || cols < 7
            ? elements.title
            : elements.prevTitle,
          elements.title,
          elements.nextTitle.disabled || cols > 4
            ? elements.title
            : elements.nextTitle,
          elements.title,
      ...
      function getBottomLineElement(index) {
        return bottomElements.length
          ? bottomElements[
              Math.floor(index / Math.ceil(cols / bottomElements.length))
            ]
          : null;
      }
        ...
        items
          ...
          .forEach(function (item, index, array) {
            ...
            if (index === array.length - 1) {
              item.neighbors.bottom =
                getBottomLineElement(i) || getTopLineElement(i) || array[0];
            } else {
    ...
    function setOption(option, value) {
      if (typeof options[option] === "undefined") {
        return (
          console.error("DatePickerX, setOption: Option doesn't exist.") &&
          false
        );
      }
      ...
      if (option === "minDate" || option === "maxDate") {
        if (
          ...
          if (isNaN(value)) {
            return (
              console.error("DatePickerX, setOption: Invalid date value.") &&
              false
            );
          }
        }
        ...
        return (
          console.error("DatePickerX, setOption: Option has invalid type.") &&
          false
        );
      } else if (Array.isArray(options[option])) {
        if (value.length < options[option].length) {
          return (
            console.warn("DatePickerX, setOption: Invalid option length.") &&
            false
          );
        }
    ...
    return {
      ...
      init: function (initOptions) {
        ...
        if (initiated) {
          return (
            console.error(
              "DatePickerX, init: Date picker has been already initiated."
            ) && false
          );
        }
        initiated = true;
        ...
        if (typeof initOptions !== "object") {
          console.error(
            "DatePickerX, init: Initial options must be an object."
          );
        } else {
      ...
      remove: function () {
        if (!initiated) {
          return (
            console.error(
              "DatePickerX, remove: Date picker has not been initiated yet."
            ) && false
          );
        }
      ...
      setValue: function (dt, ignoreLimits) {
        ...
        if (dt === null) {
          ...
          if (isNaN(dt)) {
            return (
              console.error(
                "DatePickerX, setValue: Can't convert argument to date."
              ) && false
            );
          }
          ...
          if (
            !ignoreLimits &&
            (dt.getTime() < getMinDate().getTime() ||
              dt.getTime() > getMaxDate().getTime())
          ) {
            return (
              console.error(
                "DatePickerX, setValue: Date out of acceptable range."
              ) && false
            );
          }
      ...
      getValue: function (timestamp) {
        !initiated &&
          console.error(
            "DatePickerX, getValue: Date picker has not been initiated yet."
          );
        ...
        return timestamp
          ...
          : value === null
          ? ""
          : getFormattedDate(new Date(value), options.format);
      },
  ...
  window.DatePickerX = {
    ...
    setDefaults: function (options) {
      if (typeof options !== "object") {
        return (
          console.error("DatePickerX, setDefaults: Invalid option type.") &&
          false
        );
      }
